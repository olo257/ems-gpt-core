...
<div class='card'><div class='label'>Stan aplikacji</div><div id='status' class='value warn'>URUCHAMIANIE</div><div id='applicationStatus' class='hint'>Wersja: … · uruchomiono: oczekiwanie</div></div><div class='card'><div class='label'>MariaDB</div><div id='db' class='value'>…</div></div><div class='card'><div class='label'>Dane z HA</div><div id='ha' class='value'>…</div></div><div id='executorCard' class='card clickable'><div class='label'>Wykonawca · kliknij, aby sterować</div><div id='executor' class='value warn'>…</div><div id='executorActions' class='executor-actions'><button id='executorLive' class='apply'>Włącz LIVE</button> <button id='executorOff' class='danger'>Wyłącz</button><div id='executorMessage' class='hint'></div></div></div></div><br>
...
async function tick(){try{const r=await fetch('api/status');if(!r.ok)throw new Error('HTTP '+r.status);const s=await r.json();const labels={RUNNING:'URUCHOMIONO',STARTING:'URUCHAMIANIE',DEGRADED:'TRYB OGRANICZONY',ERROR:'BŁĄD'};status.textContent=labels[s.status]||s.status||'BŁĄD';status.className='value '+(s.status==='RUNNING'?'ok':s.status==='STARTING'||s.status==='DEGRADED'?'warn':'no');const started=s.started_at?new Date(s.started_at).toLocaleString('pl-PL'):'brak';applicationStatus.textContent=`Wersja: ${s.version||'—'} · uruchomiono: ${started}`;applicationStatus.className='hint '+(s.started_at?'ok':'warn');db.textContent=s.database;ha.textContent=s.ha_input;executor.textContent=s.executor;slot.textContent=s.active_slot||'—';error.textContent=s.last_error||'Brak błędów';modules.innerHTML=Object.entries(s.modules).map(([k,v])=>`<span class='tag'>${k}: <b>${v}</b></span>`).join('')}catch(e){status.textContent='BRAK POŁĄCZENIA';status.className='value no';applicationStatus.textContent='Nie można odczytać stanu dodatku';applicationStatus.className='hint no'}}tick();loadView('plan');setInterval(tick,5000)
</script></body></html>"""
